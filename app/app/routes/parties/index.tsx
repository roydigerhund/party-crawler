...
export const loader: LoaderFunction = async ({ request }) => {
  ...
  const parties = db.rawData.findMany({
    where: {
      OR: [{ party: { contains: search, mode: Prisma.QueryMode.insensitive } }],
    },
    // distinct: ['party'],
    take: 20,
  });
...
export default function Index() {
  const parties = useLoaderData<RawData[]>();
              ...
              parties.map((image) => (
                <li key={image.id} className="relative">
                  <div className="group aspect-w-10 aspect-h-10 block w-full overflow-hidden rounded-lg bg-gray-100 focus-within:ring-2 focus-within:ring-indigo-500 focus-within:ring-offset-2 focus-within:ring-offset-gray-100">
                    ...
                    >
                      <span className="sr-only">View details for {image.party}</span>
                    </a>
                  </div>
